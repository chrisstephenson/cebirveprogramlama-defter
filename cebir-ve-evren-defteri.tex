...
\begin{center}
{\Large \bf{Nesin Köyleri Cebir ve Programlama Yazokulu 2027 - Cebir}}
...
Alıştırmaları yazmak için Alıştırmalar.rkt dosyasını açabilirsiniz.
...
\sorun{roket-yüksekliği}{Bir roket saatte 7 m/s hızla hareket edecek şekilde havalanıyor. Roketin kalktığı andan itibaren geçen süreyi alan ve roketin yüksekliğini veriyor. Bu sorun için, Roket.rkt dosyasını kullanabilirsiniz.}

\sorun{bahçe-alanı}{Fonksiyon Çim alanın yüksekliğini ve genişliğini alsın, alanını versin. (Unutma: alan = uzunluk * genişlik!). Bu sorun için, Alıştırmalar.rkt dosyasını kullanabilirsiniz.}

\sorun{kırmızı-kare}{Bu fonksiyon girdi olarak bir sayı (karenin kenar uzunluğu) alsın ve çıktı olarak içi dolu kırmızı bir kare versin. Bu sorun için, Alıştırmalar.rkt dosyasını kullanabilirsiniz.}
...
nx: Başka nesnenin x-koordinatı\\ 
ny: Başka nesnenin y-koordinatı\\
Oyuncu ve nesne arasındaki piksel mesafesi 50’den az mı? (bu fonksiyonu yazmak için yazdığın \texttt{mesafe} fonksiyonunu kullanabilirsin)}
...
\begin{center}
{\Large \bf{Nesin Köyleri Cebir ve Programlama Yazokulu 2027 - Evren}}
...
\hline			
\vspace{0.5cm}
\texttt{(STRUCT pasta (renk mesaj-rengi kat mesaj yarı-çap ))}\\[10ex]
\hline
\end{tabular}
...
\tasarimtablosu{v+}{iki vektörün toplamını hesaplar}

\tasarimtablosu{v-}{iki vektörün farkını hesaplar}

\tasarimtablosu{v.}{iki vektörün dot productu hesaplar}

\tasarimtablosu{v-mag}{bir vektörün uzunluğunu hesaplar}

\tasarimtablosu{v-scale}{birinci vektörü ikinci vektörün x ve y ile scale eder}
%scale a vector by another vector - use to bounce
...
%*** fill this in as examples for the students
\noindent
\begin{tabular}{| p{16.5cm}  |  }
\hline			
Sahne 1\\
...
\addcontentsline{toc}{section}{Benin Oyunum - Check List}
\section*{Benin Oyunum - Check List}
\begin{itemize}
	\item evren-güncelle  fonksiyonu ve altındaki fonksiyonlar tasarla ve test
	\item Oyun Sahneleri çiz
	\item Ekrandaki nesneleri listele
	\item evren ve altındaki veri yapılarını tasarla
	\item evren-çiz fonksiyonu ve altındaki fonksiyonlar tasarla ve test
	\item evren-güncelle  fonksiyonu ve altındaki fonksiyonlar tasarla ve test
	\item evren-tuş  fonksiyonu ve altındaki fonksiyonlar tasarla ve test
	\item evren-fare  fonksiyonu ve altındaki fonksiyonlar tasarla ve test
	\item evren-güncelle-etkileşim  fonksiyonu ve altındaki fonksiyonlar tasarla ve test
	\item Oyununu göster
...
\tasarimtablosu{evren-güncelle-fiziksel}
...
\section*{Faydalı Racket fonksiyonları}
\begin{tabular}{| p{4cm} | p{8cm} | p{4cm} |  }
\hline
\large \bf{Fonksiyon ismi}&\large \bf{Giriş veri tip(ler)i}&\large \bf{Sonuç veri tipi}\\
\hline
\hazirfonkspec{+}{sayı sayı }{sayı}{İki sayını toplamı}
\hazirfonkspec{-}{sayı sayı }{sayı}{İkinci sayının birincisinden çıkartılması}
\hazirfonkspec{*}{sayı sayı }{sayı}{İki sayını çarpımı}
\hazirfonkspec{/}{sayı sayı }{sayı}{Birincı sayının ikincisiyle bölünmesi }
\hazirfonkspec{quotient}{tamsayı tamsayı }{tamsayı}{Birincı sayının ikincisiyle bölünmesi, tamsayı olarak }
\hazirfonkspec{modulo}{tamsayı tamsayı }{tamsayı}{Birincı sayının ikincisiyle bölünmesinden kalanı }
\hazirfonkspec{=}{sayı sayı }{boolean}{İki sayı eşit mi?}
\hazirfonkspec{>}{sayı sayı }{boolean}{Birinci sayı ikincisinden büyük mü?}
\hazirfonkspec{>=}{sayı sayı }{boolean}{Birinci sayı ikincisinden büyük ya da eşit mi?}
\hazirfonkspec{<}{sayı sayı }{boolean}{Birinci sayı ikincisinden küçük mü?}
\hazirfonkspec{>=}{sayı sayı }{boolean}{Birinci sayı ikincisinden küçük ya da eşit mi?}
\hazirfonkspec{and}{boolean boolean }{boolean}{Her iki boolean değer doğru mu?}
\hazirfonkspec{or}{boolean boolean }{boolean}{İki boolean değerinden en az biri doğru mu?}
\hazirfonkspec{not}{boolean }{boolean}{Verilen boolean değerin doğruysa doğru değil, yoksa doğru}
\end{tabular}

\newpage
\addcontentsline{toc}{section}{Cebir ve Evren  için hazır fonksiyonlar}
\section*{Cebir ve Evren için teachpack'in sağladığı fonksiyonlar}
\begin{tabular}{| p{4cm} | p{8cm} | p{4cm} |  }
\hline
\large \bf{Fonksiyon ismi}&\large \bf{Giriş veri tip(ler)i}&\large \bf{Sonuç veri tipi}\\
\hline
\hazirfonkspec{string=?}{string string }{boolean}{İki string eşit ise doğru, yoksa yanlış}
\hazirfonkspec{string-append}{string string }{string}{İki string arka arkaya ekliyor}
\hazirfonkspec{number->string}{sayı }{string}{Bir sayı string'e çeviriyor}
\hazirfonkspec{square}{sayı string string}{imaj}{Verilen çapta verilen \texttt{\"solid\"} ya da \texttt{\"outline\"} tarzda, verilen renkte bir kare imajı yaratıyor. }
\hazirfonkspec{circle}{sayı string string}{imaj}{Verilen çapta verilen \texttt{\"solid\"} ya da \texttt{\"outline\"} tarzda, verilen renkte bir daire imajı yaratıyor. }
\hazirfonkspec{ellipse}{sayı sayı string string}{imaj}{Verilen x ve y boyutlarıyla \texttt{\"solid\"} ya da \texttt{\"outline\"} tarzda, verilen renkte bir elips imajı yaratıyor. }
\hazirfonkspec{triangle}{sayı string string}{imaj}{Verilen çapta verilen \texttt{\"solid\"} ya da \texttt{\"outline\"} tarzda, verilen renkte bir üçgen imajı yaratıyor. }
\hazirfonkspec{scale}{sayı imaj}{imaj}{Verilen imaj verilen sayıya göre büyütüp küçültmek}
...
\hazirfonkspec{bitmap}{string}{imaj}{Verilen imaj dosyası file-path'tan imaj yaratıyor}
...
\hazirfonkspec{random}{sayı sayı}{sayı}{Birinci tamsayıdan başlayan, ikinci tamsayıdan küçük olan taslantısal bir tamsayı}
...
\section*{Evren için teachpack'in sağladığı ek Fonksiyonlar}
\begin{tabular}{| p{4cm} | p{8cm} | p{4cm} |  }
\hline
\large \bf{Fonksiyon ismi}&\large \bf{Giriş veri tip(ler)i}&\large \bf{Sonuç veri tipi}\\
\hline
...
\hazirfonkspec{place-text/v}{v string sayı color imaj }{imaj}{Vektörün belirlediği yerde verilen metin verilen renkle verilen boyutta sahneye (imaja) yerleştirmek}
\hazirfonkspec{SES}{herhangi string }{herhangi}{stringde belirtilen .wav dosyasını çaldırıp birinci parametresi olduğu gibi döndürmek. Genel olarak evren olur, başka veri tipi de olabilir}
\hazirfonkspec{range}{sayı sayı sayı}{sayı listesi}{Birinci tamsayıdan başlayan, üçüncü tamsayı büyüklükte adımlarla, ikinci tamsayıdan küçük olan tamsayıların listesi}
\end{tabular}
\newpage
\section*{Liste fonksiyonları}
\begin{tabular}{| p{4cm} | p{8cm} | p{4cm} |  }
\hline
\large \bf{Fonksiyon ismi}&\large \bf{Giriş veri tip(ler)i}&\large \bf{Sonuç veri tipi}\\
\hline
\hazirfonkspec{empty?}{liste}{mantıksal}{Liste boş mu?}
\hazirfonkspec{first}{liste}{liste elemanı}{Listenin birinci elemanı}
\hazirfonkspec{rest}{liste}{liste}{Verilen liste, birinci elemanı hariç}
\hazirfonkspec{list}{elemanlar ...}{liste}{Verilen elemanların listesi}
\hazirfonkspec{map}{liste (fonksiyon:liste elemanı -> yeni değer)}{yeni değerlerin listesi}{Verilen fonksiyon kullanarak, verilen listesinin her elemanından üretilen yeni değerlerin listesi}
\hazirfonkspec{filter}{liste (fonksiyon:liste elemanı -> boolean)}{yeni değerlerin listesi}{Verilen fonksiyon kullanarak, verilen listesinin her elemanından üretilen boolean doğru olan değerlerin listesi}
\hazirfonkspec{foldr}{liste birikim-değeri (fonksiyon:liste elemanı birikim değeri -> birikim-değeri)}{yeni değerlerin listesi}{Verilen fonksiyon kullanarak, verilen listesinin sağdan başlayan birikimi}

\hazirfonkspec{range}{sayı sayı sayı}{sayı listesi}{Birinci tamsayıdan başlayan, üçüncü tamsayı büyüklükte adımlarla, ikinci tamsayıdan küçük olan tamsayıların listesi}
\end{tabular}
\newpage
\section*{2027 Eğitim Takımı}
\addcontentsline{toc}{section}{2027 Eğitim Takımı}
Hocalar
\begin{itemize}
\item Chris Stephenson \url{https://chrisstephenson.org/}
\item Mehmet Gençer \url{https://www.mgencer.com/}
\item Banu Yobaş
%\item Tuğba Dalyan
\end{itemize}
Asistanlar
...
\item Ümmü Gülsüm Yıldırım

\item Zehra Naz Çiftçi
